...
# %% auto 0
__all__ = ['HierarchicalReconciliation', 'HierarchicalForecast']
...
class HierarchicalForecast:
    ...
    def __init__(
        ...
        """
        The `core.HierarchicalForecast` class allows you to efficiently fit multiple Neural-, Stats- or
        MLForecast models for large sets of time series and hierarchically reconciled them.

        Parameters
        ----------
        models : dict
            Dictionary that maps the aggregation tags to an instantiated list of models
            that will be fitted to the time series.
        reconcilers : list
            List of instantiated classes of the [reconciliation methods](https://nixtla.github.io/hierarchicalforecast/methods.html) module.
        tags_te : dict (default=None)
            Each key is a temporal aggregation level and its value contains columns associated to that level.

        Returns
        -------
        self : HierarchicalForecast
            Returns instantiated `HierarchicalForecast` class.
        """
        # Check temporal tags validity
        self._check_tags(models, tags_te)

        self.models = models
        self.reconcilers = reconcilers
        self.tags_te = tags_te

    @staticmethod
    def _check_tags(
        models: dict[str, list[Any]], tags: Optional[dict[str, np.ndarray]] = None
    ):
        """
        Check tags validity.
        """
        if tags is not None:
            missing_models = set(tags.keys()) - set(models.keys())
            if len(missing_models) > 0:
                raise ValueError(
                    f"You should include a model for the following aggregation tags: {reprlib.repr(missing_models)}"
                )
            missing_tags = set(models.keys()) - set(tags.keys())
            if len(missing_tags) > 0:
                raise ValueError(
                    f"You have included a model for the following unknown aggregation tags: {reprlib.repr(missing_tags)}"
                )
    ...
    def fit(
        ...
        """Fit the core.HierarchicalForecast.
        ...
        ----------
        df : DataFrame
            DataFrame with columns [id_col, time_col, target_col] and exogenous variables.
        ...
        -------
        self : HierarchicalForecast
            Returns `HierarchicalForecast` class with fitted `models`.
        """
